...
namespace FuzzySelector.SmartSpammer.InferenceMachine.RuleGeneration {
    ...
    public partial class SelectionRules : InferenceMachine {
        ...
        private List<Rule> bktCombinations(int currentOffset, object aux) {
            ...
            if (firstCall) {
                ...
                foreach (int[] premises in dictionary.Keys) {
                    ...
                    result.Add(new Rule(premises, (int)Math.Round((dictionary[premises] - this.minDecision) * (SmartSpammerSettings.Default.RULE_GENERATION_NUMBER_OF_OUTCOMES - 1) / (this.maxDecision - this.minDecision))));
                }
        ...
        private Property constructRequirementOntology(RequestShape requestedShape, double domainStart, double domainEnd, out int exactlyLocation) {
            ...
            int numberOfNeighborsGeneratedOnEachSide = SmartSpammerSettings.Default.RULE_GENERATION_NUMBER_OF_NEIGHBOURS_ON_EACH_SIDE;
            ...
            if (SmartSpammerSettings.Default.RULE_GENERATION_FORCE_NEIGHBOURS) {
            ...
            if (requestedShape.shape.left != domainStart && (!SmartSpammerSettings.Default.RULE_GENERATION_FORCE_NEIGHBOURS || addLength != 0)) {
            ...
            if (requestedShape.shape.right != domainEnd && (!SmartSpammerSettings.Default.RULE_GENERATION_FORCE_NEIGHBOURS || addLength != 0)) {
